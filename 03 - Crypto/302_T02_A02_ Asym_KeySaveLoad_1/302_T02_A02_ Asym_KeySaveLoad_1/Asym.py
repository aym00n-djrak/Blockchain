...
def save_keys(keys_file_name, keys_list):
    ...
    for key in keys_list:
        key_name, private_key, public_key = key
        ...
        publice_serialized = public_key.public_bytes(
            encoding=serialization.Encoding.PEM,
            format=serialization.PublicFormat.SubjectPublicKeyInfo,
        )

        serialized_keys.append((key_name, private_serialized, publice_serialized))

    with open(keys_file_name, "wb") as savefile:
        pickle.dump(serialized_keys, savefile)
...
def load_keys(keys_file_name):
    with open(keys_file_name, "rb") as key_file:
        deserialized_keys = []
        serialized_keys_list = pickle.load(key_file)
        for serialized_key in serialized_keys_list:
            key_name, private_key_ser, public_key_ser = serialized_key
            private_key = serialization.load_pem_private_key(
                private_key_ser, password=None
            )
            public_key = serialization.load_pem_public_key(public_key_ser)
            deserialized_keys.append((key_name, private_key, public_key))
